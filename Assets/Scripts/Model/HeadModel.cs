...
namespace Snake_Game.Model
{
    public class HeadModel : IHead
    {
        ...
        public void ChangePosition(Vector3 position)
        {
            ...
            newPosition.x += position.x * Speed * Time.deltaTime;
            newPosition.z += position.y * Speed * Time.deltaTime;

            if (newPosition != _transform.position) 
            {
                _transform.position = newPosition;

                PoisitionChanged?.Invoke(oldPosition, new Vector3(position.x * Scale, 0, position.y * Scale));
            }*/
        }
    }
}
